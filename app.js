...
const LIFECYCLE_STAGES = [
  { id: 'empty', label: 'Empty', emoji: '□', color: 'bg-gray-100', description: 'No seed in slot' },
  { id: 'planted', label: 'Planted', emoji: '🌱', color: 'bg-blue-100', description: 'Seed has been sown' },
  { id: 'germinating', label: 'Germinating', emoji: '🌾', color: 'bg-green-100', description: 'Seed is sprouting' },
  { id: 'seedling', label: 'Seedling', emoji: '🌿', color: 'bg-green-200', description: 'Young plant with first leaves' },
  { id: 'vegetative', label: 'Vegetative', emoji: '🌳', color: 'bg-green-300', description: 'Rapid leaf and stem growth' },
  { id: 'flowering', label: 'Flowering', emoji: '🌸', color: 'bg-pink-200', description: 'Flowers appearing' },
  { id: 'fruiting', label: 'Fruiting', emoji: '🍅', color: 'bg-yellow-200', description: 'Fruits developing' },
  { id: 'harvesting', label: 'Harvesting', emoji: '✂️', color: 'bg-orange-200', description: 'Ready to pick' },
  { id: 'done', label: 'Done', emoji: '✓', color: 'bg-gray-200', description: 'Harvest complete' },
];
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className={`rounded-lg p-3 ${isInSowWindow(selectedPlant) ? 'bg-green-100' : 'bg-gray-100'}`}>
              ...
              <div className={`font-semibold ${isInSowWindow(selectedPlant) ? 'text-green-700' : 'text-gray-600'}`}>
                {isInSowWindow(selectedPlant) ? '✓ In Season' : 'Off Season'}
              </div>
            </div>
          </div>
    ...
    <div className="space-y-4">
      ...
      {status !== 'healthy' && (
        <div className="bg-red-50 border-l-4 border-red-400 p-4 rounded">
          <h4 className="font-semibold text-red-800 mb-2">⚠️ Common Issues</h4>
        ...
        {isEmpty ? (
          ...
          <>
            ...
            <div className="mt-6 bg-blue-50 p-4 rounded-lg border border-blue-200">
              <h3 className="font-semibold text-blue-900 mb-2">🌱 Care Tips</h3>
...
const TaskItem = ({ task, onComplete, onDelete }) => {
  ...
  const priorityConfig = {
    high: { color: 'bg-red-100', textColor: 'text-red-800', icon: '🔴' },
    medium: { color: 'bg-yellow-100', textColor: 'text-yellow-800', icon: '🟡' },
    low: { color: 'bg-green-100', textColor: 'text-green-800', icon: '🟢' },
  };
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="text-sm text-gray-600">
            {plantInfo?.commonName} • {garden?.name}
          </div>
...
const App = () => {
  ...
  const tabs = [
    ...
    { id: 'tasks', label: 'Tasks', icon: '📋', badge: pendingTasks > 0 ? pendingTasks : null },
